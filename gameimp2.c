...
typedef struct boardStackNode{
    Cell grid[8][8];
    struct boardStackNode *prev;
}BSN;
BSN* currentBoard = NULL;
Cell startBoard[8][8];
void Boardcopy(Cell grid[8][8], Cell newGrid[8][8] )
{
    for(int i=0; i<8; i++){
        for(int j=0; j<8; j++){
...
int isSame(Cell grid[8][8], Cell current[8][8]){ 
    for(int i =0; i < 8; i++){
        for (int j=0; j < 8 ; j++){
            if (grid[i][j].piece != current[i][j].piece && grid[i][j].type != current[i][j].type){
                return 0;
            }
        }
    }
...
void display(Cell grid[8][8])
{
    ...
    if(!isSame(grid,currentBoard->grid)){
        BSN *NewBoard = malloc(sizeof(BSN));
        Boardcopy(grid, NewBoard->grid);
        NewBoard -> prev = currentBoard;
...
void intializeboard(Cell grid[8][8])
{
    ...
    BSN* Board = malloc(sizeof(BSN));
    Boardcopy(grid,Board->grid);
    Boardcopy(grid,startBoard);
    Board -> prev = currentBoard;
    currentBoard = Board;
...
void undoBoard(Cell grid[8][8]){
    if( isSame(grid,startBoard)){
        return;
    }
    currentBoard = currentBoard -> prev;
    Boardcopy(currentBoard->grid,grid);
    display(grid);
...
void checkallpieces(Cell grid[8][8], int turn)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            ...
            else if(grid[i][j].piece == 'X' || grid[i][j].piece == 'B')//black
            {
                if (j >= 1 && i >= 1 && validMovepiece(grid, i, j, TOPLEFT))
                    black = 1;
...
int input(Cell grid[8][8], int turn) // turn 1 white(O,W), 0 black(X,B)
{
    ...
    if(Y == 8){
        if(isSame(grid,startBoard)==1){
            printf("Cant Undo any more");
            return -1;
        }
        undoBoard(grid);
        return 0;
